...
<script>
import { reqArticleById, reqWebMenuList ,reqArticleEditById } from "@/api";
...
export default {
  ...
  methods: {
    ...
    async getArticleData() {
      var article_id = Number(this.articleId);
      const result = await reqArticleById(
    ...
    getRuleFormData() {
      if (this.articleInfo) {
        this.ruleForm.Id = this.articleInfo.Id;
        this.ruleForm.author = this.articleInfo.author;
        this.ruleForm.author_id = this.articleInfo.author_id;
        this.ruleForm.banner = this.articleInfo.banner;
        this.ruleForm.category_id = this.articleInfo.category_id;
        this.ruleForm.content = this.articleInfo.content;
        this.ruleForm.cover = this.articleInfo.cover;
        this.ruleForm.desc = this.articleInfo.desc;
        this.ruleForm.is_bottom = this.articleInfo.is_bottom;
        this.ruleForm.is_index = this.articleInfo.is_index;
        this.ruleForm.is_original = this.articleInfo.is_original;
        this.ruleForm.is_top = this.articleInfo.is_top;
        this.ruleForm.keyword = this.articleInfo.keyword;
        this.ruleForm.link_url = this.articleInfo.link_url;
        this.ruleForm.menu_id = this.articleInfo.menu_id;
        this.ruleForm.state = this.articleInfo.state;
        this.ruleForm.title = this.articleInfo.title;
        this.ruleForm.video = this.articleInfo.video;
        if (this.ruleForm.content) {
          this.editorContent = this.ruleForm.content
        }
        ...
        if (this.ruleForm.is_index == 1) {
          this.type.push("is_index")
        } else if (this.ruleForm.is_bottom == 1) {
          this.type.push("is_bottom")
        } else if (this.ruleForm.is_top == 1) {
          this.type.push("is_top")
        }
      }
    },
    getMenuList() {
    ...
    async submitAddArticle() {
      ...
      const result = await reqArticleEditById(token, this.ruleForm);
      if (result.code === 0) {
        this.$toast({
          type: "success",
          message: "修改成功",
